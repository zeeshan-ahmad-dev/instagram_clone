<template>
  <aside
    class="md:hidden fixed z-[10000] bottom-0 border-t bg-white w-full border-r px-4 py-2 overflow-auto"
  >
    <nav class="center">
      <ul class="gap-6 center">
        <li v-for="(link, index) in MobileNavLinks" :key="index" class="center">
          <router-link
            v-if="link.isLink"
            :class="['sidebar-link', isActive(link) ? 'font-bold' : '']"
            :to="link.href"
          >
            <div>
          ...
          <div
            v-else
            @click="handleActions[link.name.toLocaleLowerCase()]?.()"
            class="sidebar-link"
          >
            <div>
              <img class="size-7" :src="getIcon(link)" :alt="link.name" />
        ...
        <li>
          <router-link
            ...
            :class="[
              'sidebar-link',
              /^\/profile\/[^/]+$/.test(route.path) ? 'font-bold' : '',
            ]"
